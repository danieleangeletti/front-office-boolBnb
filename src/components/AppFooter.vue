  ...
  data() {
    return {
      ...
      footerList: [
        {
          title: 'Assistenza',
          list: ['Centro assistenza', 'AirCover', 'Antidiscriminazione', 'Supporto alla disabilità', 'Opzioni di cancellazione', 'Segnala problema nel quartiere']
        },
        {
          title: 'Ospitare',
          list: ['Apri un Boolbnb', 'AirCover per gli host', 'Risorse per gli host', 'Forum della community', 'Ospitare responsabilmente', 'Partecipa a una lezione gratuita sull\'ospitalità']
        },
        {
          title: 'Boolbnb',
          list: ['Newsroom', 'Nuove funzionalità', 'Opportunità di lavoro', 'Investitori', 'Gift card', 'Soggiorni con Boolbnb.org']
        },
      ],
      activeIndex: 0,
    };
    ...
    <div class="container pt-5 pb-5 special-height">
      <div class="future-trips-container">
        ...
        <ul class="list-unstyled mb-5">
          <li @click="changeActiveIndex(i)" v-for="(elem, i) in destinations" class="d-inline-block pb-3 pt-3" :class="activeIndex == i ? 'active' : ''">
            <span v-if="i == 0" class="ps-0 pe-3">{{ elem }}</span>
            <span v-if="i > 0 && i < destinations.length - 1" class="ps-3 pe-3">{{
              elem
            }}</span>
